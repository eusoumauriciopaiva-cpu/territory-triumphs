...
import type { Profile, Conquest, AppRank } from '@/types';
...
export function ProfileScreen({
  ...
  // Normalização de Dados (Topo do Componente)
  const safeStats = {
    km: Number(profile?.total_km ?? 0),
    area: Number(profile?.total_area ?? 0),
    level: profile?.level ?? 1,
    rankKey: (profile?.rank || 'bronze').toLowerCase() as AppRank
  };
  const currentRank = RANK_CONFIG[safeStats.rankKey] || RANK_CONFIG['bronze'] || Object.values(RANK_CONFIG)[0];

  // Fallback de UI: Se profile for nulo, retorne loading simples
  if (!profile) {
    return (
      ...
      <div className="flex flex-col items-center mb-6">
        <div className="relative mb-4">
          ...
          <div className="absolute -bottom-2 -right-2">
            <EloBadge rank={safeStats.rankKey} size="md" showLabel={false} />
          </div>
        </div>

        <h2 className="text-2xl font-black tracking-tighter text-foreground">{profile?.name || 'Usuário'}</h2>
        {profile?.nickname && (
          <p className="text-sm text-primary font-medium">@{profile.nickname}</p>
        )}
        <p className="text-xs text-muted-foreground font-mono mt-1">
          #{profile?.unique_code || '0000'}
        </p>
        <p className="text-sm text-muted-foreground mt-1">
          Nível {safeStats.level} • {currentRank?.label || 'Explorador'}
        </p>
        ...
        <div className="flex items-center gap-3">
          <StreakBadge streak={profile?.current_streak || 0} bestStreak={profile?.best_streak || 0} />
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="bg-muted/50 px-3 py-2 rounded-md border border-border">
                      <span className="font-mono text-primary font-bold">#{profile?.unique_code || '0000'}</span>
                    </div>
                    <p className="text-[10px] text-muted-foreground">
        ...
        <div className="space-y-4">
          ...
          {showHeatmap && (
            <div className="h-64 rounded-2xl overflow-hidden border border-border">
              <ZonnaMap3D
                userPosition={safeHistory.length > 0 && safeHistory[0]?.path?.[0] ? safeHistory[0].path[0] : null}
                heatmapMode={true}
                trailColor={trailColor}
                userConquests={safeHistory}
              />
            </div>
          )}
        </div>
      ...
      {activeSection === 'history' && (
        <div>
          {safeHistory.length === 0 ? (
            ...
            <div className="space-y-2">
              {safeHistory.slice(0, 15).map((conquest, index) => {
                const dist = Number(conquest?.distance ?? 0);
                const area = Number(conquest?.area ?? 0);
                return (
                  <button
                    key={conquest?.id || index}
                    onClick={() => conquest && onShowOnMap(conquest)}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <span className="font-bold text-sm block">Território #{safeHistory.length - index}</span>
                        <span className="text-xs text-muted-foreground">
                          {isNaN(dist) ? '0.00 km' : dist.toFixed(2) + ' km'} • {conquest?.created_at ? new Date(conquest.created_at).toLocaleDateString('pt-BR') : 'Data inválida'}
                        </span>
                      </div>
                    </div>
